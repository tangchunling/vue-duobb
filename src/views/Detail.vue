  ...
  <div>
    <div class="detcontainer" v-if="detaildata">
    		...
    		<div style="width: 1000px;margin: auto;">
    			<div class="detleft">
	    			<img v-bind:src="detaildata.taobaoToken.picUrl" alt="" />
	    		</div>
	    		<div class="detright">
	    			<p class="title" v-html="detaildata.taobaoToken.title"></p>
	    			<div class="detdec">
	    				<div class="list">
	    					<label>原价</label><span class="yprice">¥{{detaildata.taobaoToken.reservePrice}}</span>
	    				</div>
	    				<div class="list">
	    					<label>促销价</label><span class="cxprice">¥<strong>{{detaildata.taobaoToken.zkPrice}}</strong></span>
	    				</div>
	    				<div class="list">
	    			...
	    			<div class="detbtn">
	    				<a v-if="detaildata.taobaoToken.couponInfo != '无'" v-bind:href="detaildata.taobaoToken.couponLink" target="_blank">领券购买</a>
	    				<a v-else v-if="detaildata.hideToken.length" v-bind:href="detaildata.hideToken[0].url" target="_blank">领券购买</a>
	    				<a v-else v-bind:href="detaildata.taobaoToken.clickUrl" target="_blank">领券购买</a>
    ...
    <div class="yhq" v-if="detaildata">
    		<div class="serchresult">
    			...
    			<span v-if="detaildata.hideToken.length">为你找到{{detaildata.hideToken.length}}张优惠券</span>
    			<span v-else>暂无隐藏优惠券</span>
    			<img src="../assets/img/right.png" alt="" style="float: right;"/>
    		</div>
    		<ul v-if="detaildata.hideToken.length">
    			<li v-for="item in detaildata.hideToken">
...
export default {
  ...
  methods: {
  		getdata: function () {
  			var _data = 'auctionId='+this.id
  			var self = this
  			this.$http.get(api + '/index.php?r=userdata/select&'+_data).then((response) => {
